...
export default class App extends React.Component {
  ...
  async handleSearch(textSearch) {
    ...
    const matchingPokemons = data.results.filter((pokemon) =>
        pokemon.name.includes(textSearch.toLowerCase())
    );
    ...
    if (matchingPokemons.length === 0) {
      ...
      const promises = matchingPokemons.map(async (pokemon) => {
  ...
  render() {
    const poke =
        this.state.search.length > 0 ? this.state.search : this.state.pokemons;
    return (
        <>
          <Container>
